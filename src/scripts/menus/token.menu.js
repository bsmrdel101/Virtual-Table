...
let tokens = [];
let defaultTokens = [
    {image: 'https://i.pinimg.com/236x/88/4a/05/884a056ba7a5a004becacbfd1bfd78fe.jpg', size: 90},
    {image: 'https://i.imgur.com/5cibmUw.png', size: 190},
    {image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSlW_xekRD291YBhLdPKYifDnF2HV74Csz0KQ&usqp=CAU', size: 390},
];
...
async function getTokenBodyData() {
    ...
    for (let token of tokens) {
        document.querySelector('.menu__body').insertAdjacentHTML('beforeend', `
            <img src=${token.image} width=${token.size}% class="menu__item menu__item--token" ondragstart="placeToken(event, '${token.size}')" size=${token.size}>
        `);
    }
}
